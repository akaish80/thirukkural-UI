...
try {
  ...
  app.post('/chat', (req, res) => {
    ...
    try {
      const logLine = JSON.stringify({ ts: Date.now(), ip: req.ip || req.connection?.remoteAddress, query, topN: topN || 10, resultCount: (result && result.results && result.results.length) ? result.results.length : (Array.isArray(result) ? result.length : (result && result.kurral ? 1 : 0)) });
      const logPath = path.join(__dirname, 'chat_queries.log');
      fs.appendFile(logPath, `${logLine}\n`, (err) => { if (err) console.warn('Failed to append chat log:', err.message); });
    } catch (e) {
